...
const DropdownMenu = ({ nearbyStations, fetchTideData, setStationName }: DropdownProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedStationId, setSelectedStationId] = useState<string | null>(nearbyStations[0].id);
            ...
            <View style={styles.modalContainer}>
              <FlatList
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    <Text style={styles.itemCheckmark}>{item.id === selectedStationId ? '✔' : ''}</Text>
